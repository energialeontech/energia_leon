import type { Metadata } from "next";
import ContactForm from "@/components/ContactForm";
import CanalesContacto from "@/components/CanalesContacto";

export const metadata: Metadata = {
  title: "Contacto | Asesoría Energética León",
  description:
    "Contáctanos para tu estudio energético gratuito. Te contactaremos, escribiremos por WhatsApp o rellenas el formulario.",
};

const canales = [
  {
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.58 3.49 2 2 0 0 1 3.55 1.27h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L7.91 8.91a16 16 0 0 0 6.08 6.08l.91-.91a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7 2 2 0 0 1 1.72 2z" />
      </svg>
    ),
    titulo: "Te contactaremos",
    valor: "+34 610 39 62 08",
    desc: "Llámanos o déjanos tu número y nosotros te contactaremos.",
    href: "tel:+34610396208",
    id: "contacto-telefono",
  },
  {
    icon: (
      <svg width="30" height="30" viewBox="0 0 32 32" fill="currentColor">
        <path d="M26.576 5.363c-2.69-2.69-6.406-4.354-10.511-4.354-8.209 0-14.865 6.655-14.865 14.865 0 2.732 0.737 5.291 2.022 7.491l-0.038-0.070-2.109 7.702 7.879-2.067c2.051 1.139 4.498 1.809 7.102 1.809h0.006c8.209-0.003 14.862-6.659 14.862-14.868 0-4.103-1.662-7.817-4.349-10.507l0 0zM16.062 28.228h-0.005c-0 0-0.001 0-0.001 0-2.319 0-4.489-0.64-6.342-1.753l0.056 0.031-0.451-0.267-4.675 1.227 1.247-4.559-0.294-0.467c-1.185-1.862-1.889-4.131-1.889-6.565 0-6.822 5.531-12.353 12.353-12.353s12.353 5.531 12.353 12.353c0 6.822-5.53 12.353-12.353 12.353h-0zM22.838 18.977c-0.371-0.186-2.197-1.083-2.537-1.208-0.341-0.124-0.589-0.185-0.837 0.187-0.246 0.371-0.958 1.207-1.175 1.455-0.216 0.249-0.434 0.279-0.805 0.094-1.15-0.466-2.138-1.087-2.997-1.852l0.010 0.009c-0.799-0.74-1.484-1.587-2.037-2.521l-0.028-0.052c-0.216-0.371-0.023-0.572 0.162-0.757 0.167-0.166 0.372-0.434 0.557-0.65 0.146-0.179 0.271-0.384 0.366-0.604l0.006-0.017c0.043-0.087 0.068-0.188 0.068-0.296 0-0.131-0.037-0.253-0.101-0.357l0.002 0.003c-0.094-0.186-0.836-2.014-1.145-2.758-0.302-0.724-0.609-0.625-0.836-0.637-0.216-0.010-0.464-0.012-0.712-0.012-0.395 0.010-0.746 0.188-0.988 0.463l-0.001 0.002c-0.802 0.761-1.3 1.834-1.3 3.023 0 0.026 0 0.053 0.001 0.079l-0-0.004c0.131 1.467 0.681 2.784 1.527 3.857l-0.012-0.015c1.604 2.379 3.742 4.282 6.251 5.564l0.094 0.043c0.548 0.248 1.25 0.513 1.968 0.74l0.149 0.041c0.442 0.14 0.951 0.221 1.479 0.221 0.303 0 0.601-0.027 0.889-0.078l-0.031 0.004c1.069-0.223 1.956-0.868 2.497-1.749l0.009-0.017c0.165-0.366 0.261-0.793 0.261-1.242 0-0.185-0.016-0.366-0.047-0.542l0.003 0.019c-0.092-0.155-0.34-0.247-0.712-0.434z"></path>
      </svg>
    ),
    titulo: "WhatsApp",
    valor: "+34 610 39 62 08",
    desc: "Escríbenos ahora y te respondemos en minutos.",
    href: "https://wa.me/34610396208?text=Hola%2C%20quiero%20mi%20estudio%20energ%C3%A9tico%20gratis",
    id: "contacto-whatsapp",
  },
  {
    icon: (
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" />
        <polyline points="22,6 12,13 2,6" />
      </svg>
    ),
    titulo: "Email",
    valor: "info@energialeon.com",
    desc: "Envíanos un email con tu factura adjunta.",
    href: "mailto:info@energialeon.com",
    id: "contacto-email",
  },
];

export default function ContactoPage() {
  return (
    <>
      {/* Hero */}
      <section style={{ background: "linear-gradient(135deg, #1A0505 0%, #3D0C0C 100%)", padding: "5rem 1.5rem", textAlign: "center" }}>
        <div className="container">
          <span className="badge" style={{ marginBottom: "1rem", background: "rgba(245,158,11,0.15)", color: "#F59E0B" }}>
            Estudio gratuito · Sin compromiso
          </span>
          <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: "clamp(2rem, 5vw, 3rem)", color: "white", marginBottom: "1rem" }}>
            Hablemos de tu{" "}
            <span style={{ background: "linear-gradient(135deg, #F59E0B, #FCD34D)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}>
              factura energética
            </span>
          </h1>
          <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "1.1rem", maxWidth: "520px", margin: "0 auto", lineHeight: 1.7 }}>
            Elige cómo quieres contactarnos. Responderemos en menos de 24 horas.
          </p>
        </div>
      </section>

      {/* Canales rápidos */}
      <section style={{ background: "#FFF5F5", borderBottom: "1px solid #FECACA", padding: "2rem 1.5rem" }}>
        <div className="container">
          <CanalesContacto canales={canales} />
        </div>
      </section>

      {/* Formulario */}
      <section className="section">
        <div className="container" style={{ display: "grid", gridTemplateColumns: "1fr 1.5fr", gap: "3rem", alignItems: "flex-start" }}>
          {/* Info lateral */}
          <div>
            <span className="badge" style={{ marginBottom: "1rem" }}>Formulario de contacto</span>
            <h2 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: "1.875rem", color: "#1C1917", marginBottom: "1rem", lineHeight: 1.2 }}>
              Pide tu estudio <span className="text-gradient">energético gratis</span>
            </h2>
            <p style={{ color: "#6B7280", fontSize: "0.95rem", lineHeight: 1.75, marginBottom: "2rem" }}>
              Rellena el formulario y adjunta tu factura (opcional). En menos de 24 h te contactamos con el análisis.
            </p>

            {/* Pasos rápidos */}
            <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
              {[
                { paso: "1", texto: "Rellena tus datos de contacto" },
                { paso: "2", texto: "Adjunta tu factura (opcional pero recomendado)" },
                { paso: "3", texto: "Te contactaremos en menos de 24 horas" },
              ].map((item) => (
                <div key={item.paso} style={{ display: "flex", alignItems: "center", gap: "0.875rem" }}>
                  <div style={{ width: "2rem", height: "2rem", minWidth: "2rem", background: "linear-gradient(135deg, #C62828, #8B1A1A)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <span style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 900, fontSize: "0.75rem", color: "white" }}>
                      {item.paso}
                    </span>
                  </div>
                  <span style={{ color: "#374151", fontSize: "0.9rem" }}>{item.texto}</span>
                </div>
              ))}
            </div>
          </div>

          {/* Formulario completo */}
          <div style={{ background: "white", borderRadius: "1.25rem", padding: "2rem", border: "1px solid #FECACA", boxShadow: "0 4px 20px rgba(198,40,40,0.06)" }}>
            <ContactForm variant="full" />
          </div>
        </div>
      </section>
    </>
  );
}
